...
function loadCheckInData(startDate, endDate) {
  // Load the CSV data
...
function pieChart(startDate, endDate) {
  ...
  const existingSvg = d3.select('#piechart svg')
  ...
  const height = svg.node().getBoundingClientRect().height - 50
  ...
  svg
    ...
    .duration(2000) // Specify duration in milliseconds
...
function updatePieChart(startDate, endDate) {
  if (checkinData == null) {
    loadCheckInData(startDate, endDate)
  } else {
    pieChart(startDate, endDate)
  }
}
